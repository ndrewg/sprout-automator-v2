...
> **Scope refinement, 2026-09-03 (independent architecture review, `improve-codebase-architecture`).** Re-reading the code confirms this defect and widens it by one caller. `services/scheduler.ts` registers `fireCron` as `() => void fireCron(...)` (no `.catch`), and the risky `await resolveHolidayDecision(now)` call happens **before** `fireCron`'s own try block starts — so today, wrapping only the gazette read closes the throw path, but it does so by accident of there being nothing else awaited before that try block. The doc comment at the top of `fireCron` already claims "must never throw across the cron boundary"; the code doesn't yet guarantee that independent of what gazette.ts does. **Widen the fix:** wrap the whole body of `fireCron` (from the `resolveHolidayDecision` call through the end) in one try/catch, not just the gazette seam — so the boundary's own contract holds regardless of what a future awaited call inside it does. `retry.ts` and `notifications.ts`'s sweep are already safe, but only because each happens to sit inside an outer try/catch that wasn't written for this purpose — that's an accidental isolation, not a designed one. Same gate, same tests, one extra assertion: a `resolveHolidayDecision` that throws (not just the DB-error path) must not propagate out of `fireCron`.
...
> **Confirmed independently, 2026-09-03 (architecture review).** `services/retry-registry.ts` is a legitimate deep module (real in-memory scheduling state, not a thin wrapper — inlining it would just move the cron-task lifecycle logic into `retry.ts`/`notifications.ts`). The friction is exactly the interface gap this section already names: `clearPendingRetries()` exists but is opt-in per test file — `test/integration/retry.test.ts` calls it four times, `executor-failure-backstop.test.ts` never does. No new fix needed beyond what's specced above; consider making the reset a shared `afterEach` in the integration project's setup (rather than a per-file habit) so the next test file that schedules a retry can't reintroduce the same leak by omission.
