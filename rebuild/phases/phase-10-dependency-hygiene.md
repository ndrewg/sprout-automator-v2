...
> ⚠️ **As-built (found 2026-08-24):** the phase was implemented end to end; the following reality diverged from the spec and is now the record.
> - **The audit baseline was 18, not 17** (7 moderate / 10 high / 1 critical) when the work started; the live advisory DB had grown one entry since the phase was written. Final state: **both `pnpm audit --audit-level=high` and `--prod` report "No known vulnerabilities found".**
> - **10A was not a pure version bump.** vitest 4 removed `vitest.workspace.ts` and the `poolOptions.forks.singleFork` config — the two projects were migrated into `vitest.config.ts` `test.projects` (`maxWorkers: 1` for integration serial execution). A stale lockfile resolved vitest 4's `^6||^7||^8` vite peer to the leftover 5.4.21, so `vite@^8.2.2` was added as an explicit devDependency. The `Mock` type (not `ReturnType<typeof vi.fn>`) was needed in `otp-acquisition.test.ts` because vitest 4's `Mock<Procedure>` default collapsed `mockImplementation`'s callback type and tripped oxlint's typeAware `no-misused-promises`.
> - **10B's real breaking change was `DrizzleQueryError`** (introduced 0.44): every driver error is now wrapped, with the pg error on `.cause` — so `err.code === "23505"` silently stopped matching and the race guard returned 500 instead of 409. The integration suite proved it red; the fix is a shared `src/lib/pg-errors.ts` (`isUniqueViolation`/`pgErrorCode` walking the cause chain) used by `runs.ts` and `auth.ts`. The `steps` jsonb `$type` annotation, the partial index, and `ON DELETE CASCADE` compiled and behaved unchanged. Migrations from empty onto a scratch DB produced tables/indexes/constraints identical to `sprout`, and the race-guard test was proven to fail when the partial index is dropped.
> - **10C's runtime set was larger than the four listed.** The audit also flagged `deepmerge-ts` (via `mailparser>html-to-text@10.0.0`, HIGH) and `uuid` (via `node-cron`, moderate). `node-cron` had to go **3 → 4** (major) to drop `uuid`; its `stop()` now types `void | Promise<void>` (inline tasks return `void` at runtime) and `scheduler.ts` marks it with `void` explicitly for `no-floating-promises`. **Two scoped `pnpm.overrides` were required** — `esbuild@~0.18.20 → 0.28.1` (the deprecated, unmaintained `@esbuild-kit` chain inside `drizzle-kit` 0.31, no upstream fix possible) and `html-to-text@10.0.0 → 10.0.1` (mailparser pins 10.0.0 exactly; its `deepmerge-ts ^7.1.5` was the last prod finding) — each reported explicitly per the phase's override rule. pnpm 11 quirk: adding `overrides:` does not re-resolve on `pnpm install`; `pnpm update <transitive>` forced the overridden edges.
> - **10D implemented as recommended** (`--prod` blocking + full audit non-blocking), recorded in `reference/supply-chain-and-ci.md` § 4, and `ci.yml` now has both audit steps. `@types/node-cron@3` was removed (node-cron 4 ships its own types).
