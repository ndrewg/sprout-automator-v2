...
- `clientIp` returns the `CF-Connecting-IP` value when it is a valid IPv4/IPv6 address.
- `clientIp` **ignores** a malformed or empty `CF-Connecting-IP` and falls back to `req.ip`.
- Two requests with **different** `CF-Connecting-IP` values do not share an auth budget; two with the **same** value do. This is the property the gate exists for.
