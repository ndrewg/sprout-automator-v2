...
> > > ⚠️ **As-built (found 2026-08-13, hardening pass — `reviews/phase-8-cf-peer-hardening.md` F1–F5):** the `TRUSTED_CLOUDFLARE_PEERS` gate gained five fixes so it cannot fail *silently* — every one of them was "the day you enable the tunnel" risk. (1) **F1** — every parsed entry now runs through the same `normalizePeer` as the socket peer, so `::ffff:172.20.0.4` in config matches a `172.20.0.4` peer and the reverse; the parser also lowercases, and `normalizePeer` only collapses the mapped prefix when the remainder is genuinely IPv4 (`::ffff:0:1` stays IPv6). (2) **F2** — each entry is validated (`node:net isIP` plus CIDR prefix arithmetic) and a malformed one **refuses to boot** in `config.ts` `loadConfig()` — same `Invalid environment configuration:` shape as `SIGNUP_ALLOWED`, naming the 1-based position and the fix — instead of yielding a non-empty set that matches nothing. (3) **F3** — startup logs the trusted-peer **count** (`trustedCloudflarePeerCount()`); a `CF-Connecting-IP` arriving from a peer outside a *non-empty* set logs a **once-only** `warn` with the observed peer (the empty set — the normal state — never warns). (4) **F4** — entries may be **CIDR ranges** (`172.20.0.0/16`) because cloudflared's container IP decays when Docker renumbers; matching is bit arithmetic on `bigint`, no dependency added. IPv6-with-embedded-IPv4 forms other than the collapsed `::ffff:` prefix are refused rather than silently never-matched. (5) **F5** — `resetRateLimits()` now restores the **empty** set instead of re-reading config, so a test that arms a peer must re-apply it after each reset; the module-load parse is the only config-derived population. Docs updated in `DEPLOY.md` § 4.1 and `.env.example`.
>
> > > > ⚠️ **As-built (found 2026-08-13, tester round 4 — `reviews/phase-8-addendum.md` B8–B10):** three more `TRUSTED_CLOUDFLARE_PEERS` values that trusted *more* than the operator intended are now refused at boot in `lib/trusted-peers.ts`, in the same F2 stance — the config-validation philosophy this gate is built on. (1) **B8** — a `/0` prefix (`0.0.0.0/0`, `::/0`) has mask 0 and matches EVERY peer of the family: it re-opened the spoofing hole while reporting as armed (`trustedCloudflarePeerCount` = 1). It now refuses to boot, naming the tunnel's own address or network as the fix. (2) **B10** — a range with host bits set (`172.20.0.5/16`) was silently normalized to the whole `172.20.0.0/16`, widening trust 65k-fold. It now refuses to boot and names both corrected forms: the network (`172.20.0.0/16`) or the single address with its full mask (`172.20.0.5/32`). (3) **B9** — the boot error interpolated the raw entry, so a value containing a newline split the refusal across log lines; echoed entries are now escaped (`JSON.stringify`) and capped at 60 chars. `DEPLOY.md` § 4.1 shows the two correct forms.
