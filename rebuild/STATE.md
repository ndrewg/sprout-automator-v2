...
1. **Phase L — lint hardening.** Small, and it protects everything after it. Audit (2026-08-08) found linting is configured to almost nothing and wired to nothing: the frontend lists two oxlint rules (which *replaces* the default set, disabling the correctness category), the backend has no linter at all, and `pnpm lint` runs in neither CI nor pre-commit. See `phases/phase-L-lint-hardening.md`.
...
- **Linting is nominally present and effectively absent.** oxlint exists on the frontend with two hand-listed rules (which suppresses the whole default set); the backend has none; `pnpm lint` is in neither CI nor pre-commit. Phase L fixes this. Note the bug that triggered the audit — a fragment-shorthand missing `key` in `RunsPanel` — is **not** caught by oxlint even with the correctness category and `react/jsx-key` enabled; the standard rule only checks `<>…</>` under `checkFragmentShorthand`, off by default in ESLint too.
