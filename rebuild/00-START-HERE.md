...
4. **For Phase 3 UI, the `ui-ux-pro-max` skill is available** (a Python-CLI design-intelligence skill: design-system generation + `Platform: Web` responsive/UX guidelines). Phase 3 spells out how to use it. The dashboard **must be responsive** — colleagues use it on phones — so use the skill's `Platform: Web` rules and skip its React-Native/native-only ones.
